...
- Tray orchestration lives in `internal/tray`.
- Collector lifecycle control is handled by `internal/app.Controller`.
- The app loop still runs in-process.
- The tray icon is embedded from:
  - `internal/tray/assets/icon32x32.png` (active tray icon)
  - `internal/tray/assets/icon16x16.png` (reserved for future use)
